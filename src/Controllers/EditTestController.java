...
public class EditTestController implements Initializable {
    ...
    @Override
    /*
    Creates both the test table and the edited test table, also adds tags filtering
     */
    ...
    /*
    Takes in a test that needs to be edited and populates the edited test table
     */
    public void receiveTest(Test receivedTest)
    {
        test = receivedTest;
        QuestionsRepository qData = new QuestionsRepository();


        String str = test.QuestionList;
        str = str.replace("[", "");
        str = str.replace("]", "");
        str = str.replaceAll(" ", "");
        ...
        {
            questionList.add(qData.getQuestion(Integer.parseInt(qID)));
        }

        for (Question q:questionList)
        {
            QuestionTestRows.add(q);
        }

        testName.setText(test.TestName);
        editedTestLbl.setText("Editing Test ID: " + test.TestID);
    }

    /*
    Add question to the currently in editing test table
     */
    ...
    /*
    Remove question from the currently in editing test table
     */
    ...
    public void saveTest(ActionEvent event)
    {
        ...
        if (isMarkedManually.isSelected() == true)
        {
            markedManually = 1;
        }
        else
        ...
        Test testToSend = new Test(test.TestID,questionIdList.toString(), testName.getText());
        TestsRepository TestsRepo = new TestsRepository();
        if (Validation(testToSend))
        {
            TestsRepo.editTest(testToSend);
            showMessageDialog(null, "Test Edited");
